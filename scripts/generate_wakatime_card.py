...
def remap_and_sort(
    ...
    for language in spire_languages:
        name = str(language.get("name", "Other"))
        project_seconds = float(language.get("total_seconds", 0))
        spire_total += project_seconds
        seconds[name] = max(0.0, seconds[name] - project_seconds)

    seconds["C++"] += spire_total
    ...
    return (pinned + rest)[:MAX_LANGUAGES]
...
def render_svg(languages: list[dict[str, Any]]) -> str:
  ...
  <style>
    .title {{ fill: #f05237; font: 700 25px "Segoe UI", Ubuntu, sans-serif; }}
    .language {{ fill: #ffffff; font: 400 17px "Segoe UI", Ubuntu, sans-serif; }}
    .hours {{ fill: #ffffff; font: 400 16px "Segoe UI", Ubuntu, sans-serif; }}
    .empty {{ fill: #ffffff; font: 400 15px "Segoe UI", Ubuntu, sans-serif; }}
  </style>
  <rect x="0.5" y="0.5" width="499" height="{height - 1}" rx="5" fill="#09131b" stroke="#0e2634"/>
